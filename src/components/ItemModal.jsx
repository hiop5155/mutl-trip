...
export default function ItemModal({ item, dayColor, travelers, currentUser, selfTraveler, uidToName, onClose, onUpdate }) {
  const { t } = useI18n();
  const [noteText, setNoteText] = useState("");
  const noteAuthor = selfTraveler || travelers[0] || "Traveler";
  ...
  const addNote = () => {
    ...
    const n = [...(item.notes || []), {
      id: Date.now(), author: noteAuthor, text: noteText,
  ...
  // All members see all notes; only the creator can delete their own.
  const allNotes = item.notes || [];
  const resolveName = (n) => (uidToName && n.creatorUid && uidToName[n.creatorUid]) || n.author || "?";
    ...
    <div onClick={onClose} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.45)", zIndex: 1000, display: "flex", alignItems: "flex-end", justifyContent: "center", backdropFilter: "blur(2px)" }}>
      <div onClick={e => e.stopPropagation()} style={{ background: "var(--bg-main)", borderRadius: "20px 20px 0 0", width: "100%", maxWidth: 500, maxHeight: "85vh", overflowY: "auto", animation: "slideUp 0.3s ease" }}>
        ...
        <div style={{ padding: 20 }}>
            ...
            {editingMap ? (
              <div>
                <textarea value={mapVal} onChange={e => setMapVal(e.target.value)} placeholder={t("trip.map_placeholder")} autoFocus rows={4}
                  style={{ width: "100%", padding: "9px 12px", border: "1px solid var(--border-main)", borderRadius: 10, fontSize: 12, outline: "none", marginBottom: 8, color: "var(--text-main)", resize: "vertical", lineHeight: 1.6, fontFamily: "inherit", background: "var(--input-bg)" }} />
                <div style={{ display: "flex", gap: 6 }}>
                  <button onClick={() => { const raw = mapVal.trim(); const urlMatch = raw.match(/https?:\/\/[^\s\n]+/); const url = urlMatch ? urlMatch[0] : raw; onUpdate({ ...item, mapUrl: url }); setEditingMap(false); }}
          ...
          {/* Shared notes — all members see, only creator can delete */}
          <div style={{ marginBottom: 16 }}>
            <div style={{ fontSize: 13, fontWeight: 600, color: "var(--text-main)", marginBottom: 10 }}>{t("trip.notes_title")}</div>
            {allNotes.length === 0 && <div style={{ fontSize: 12, color: "var(--text-muted)", padding: "12px 0", textAlign: "center" }}>{t("trip.no_notes")}</div>}
            {allNotes.map(n => (
              <div key={n.id} style={{ background: "var(--bg-card)", borderRadius: 10, padding: "10px 14px", marginBottom: 8, boxShadow: "0 1px 3px rgba(0,0,0,0.04)", borderLeft: `3px solid ${TC[resolveName(n)] || "#C4A882"}` }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
                  <span style={{ fontSize: 11, fontWeight: 600, color: "var(--text-muted)" }}>{resolveName(n)}</span>
                  <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                    <span style={{ fontSize: 10, color: "var(--text-muted)" }}>{n.time}</span>
                    {(!n.creatorUid || n.creatorUid === currentUser?.uid) && (
                      <button onClick={() => onUpdate({ ...item, notes: (item.notes || []).filter(x => x.id !== n.id) })}
          ...
          <div style={{ background: "var(--bg-card)", borderRadius: 12, padding: 14, boxShadow: "0 1px 4px rgba(0,0,0,0.04)" }}>
            <div style={{ display: "flex", gap: 8 }}>
              <input value={noteText} onChange={e => setNoteText(e.target.value)} onKeyDown={e => e.key === "Enter" && addNote()}
                placeholder={t("trip.add_note_placeholder")}
                style={{ flex: 1, padding: "10px 12px", border: "1px solid var(--border-main)", borderRadius: 10, fontSize: 13, outline: "none", background: "var(--input-bg)", color: "var(--text-main)" }} />
